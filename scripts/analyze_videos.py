...
def process_video(video_path: str, output_dir: str, tmp_dir: str,
    ...
    if all_shots_deduped:
        ...
        print(f"  📸 提取 {len(all_shots_deduped)} 张截图 ...")
        extracted = 0
    ...
    if final_count > 0:
        parts.append(f"**干货截图**: {final_count} 张\n")
    parts.append("\n---\n")
...
def main():
    parser = argparse.ArgumentParser(description="批量视频画面分析 + 截图")
