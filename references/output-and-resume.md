...
- Multiple generated roles can share the same `CoursePackage`.
...
- To create one Skill from multiple courses, first run `scripts/build_multi_course_package.py`, then build the requested role from the combined workspace.
- Preserve `source_course` and `source_course_id` fields when answering from a combined package.
- Treat single-course, multi-course, and fused/domain packages as scope metadata, not Skill role names.

## Naming Rules

- If no skill name is specified, use the builder default suffix for the role.
- Add the final `-lineage` suffix to mark Skills generated by lineage-skill.
- Mentor default: `<course-slug>-mentor-lineage`.
- Expert role: `<course-slug>-expert-lineage`.
- Consultant role: `<course-slug>-consultant-lineage`.
- Practitioner role: `<course-slug>-practitioner-lineage`.
- Custom role: `<course-slug>-custom-lineage`.
- Do not add `single`, `multi`, or `fused` to the name by default; record scope in package/manifest metadata.
